...
caben, o en **laminas del espesor que quieras** (3 mm, 5 mm, 18 mm...). Cada pieza
sale **marcada con su nombre grabado** y, si quieres, con **pasadores de alineacion**,
para que luego puedas armar el modelo entero sin volverte loco.

Todo ocurre **en tu equipo**: funciona sin conexion y ningun modelo se sube a ningun
servidor. Codigo libre con licencia MIT.
...
2. Doble clic en **`Cortador.bat`**. La primera vez se instala solo (necesita
...
cd Cortador
./cortador.sh          # instala la primera vez y abre la interfaz
```
...
```bash
cortador web        # o doble clic en el .exe / en Cortador.bat
```
...
4. Pon el **volumen de tu maquina** (atajos para Ender 3, Bambu X1, Flsun V400,
   Neptune 4 Max, Modix...).
5. La **rejilla de corte se dibuja en vivo** sobre el modelo: ves cuantas piezas
   van a salir antes de cortar nada.
6. **Cortar**. Al terminar tienes la vista explosionada, el filtro por capa, el
   despiece completo y la descarga.
...
| `cortador/web/` | servidor FastAPI + panel y visor WebGL propio |
